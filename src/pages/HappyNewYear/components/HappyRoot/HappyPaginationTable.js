...
export default class HappyPaginationTable extends Comment {
    constructor(props) {
        super(props);
        let self = this;
        this.state = {
    ...
    */
    cellClick = async (record, btnFlag) => {
        await actions.HappyNewYear.updateState({
            rowData: record,
        });
    ...
    componentDidMount() {
        actions.HappyNewYear.loadList();//table数据
    }
    ...
    //提交成功
    onSubmitSuc = async ()=>{
        await actions.HappyNewYear.loadList();
        actions.HappyNewYear.updateState({showLine:false});
        Message.create({content:'单据提交成功',color:'success'});
    }
    //提交操作初始执行操作
    onSubmitStart = ()=>{
        actions.HappyNewYear.update({showLine:ture});
    }
    //提交失败回调函数
    onSubmitFail = (error)=>{
        actions.HappyNewYear.updateState({showLine:false});
    ...
    onRecallStart = ()=>{
        actions.HappyNewYear.updateState({showLine:true});
    }
    //查看方法
    onExamine = async (text, record, index)=> {
        console.log("record", record);
        await actions.HappyNewYear.updateState({rowData:record});
    ...
    onModalDel = async (delFlag)=>{
        ...
        if(delFlag){
            await actions.HappyNewYear.delItem({
                param: delData
            });
        }
        this.setState({
    ...
    render(){
        ...
        return(
            <div className='Happy-root'>
                <Header title='Happy请购单'/>
